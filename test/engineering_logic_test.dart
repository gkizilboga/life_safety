...
void main() {
  ...
  group('Mühendislik ve Kapasite Hesaplama Testleri', () {
    ...
    test('Bölüm 36 (Madde 41): Merdivenlerin yarısından azı dışarı açılıyorsa KRİTİK RİSK', () {
    ...
    test('Bölüm 20 (Madde 41): Sprinkler yoksa ve tahliye mesafesi > 10m ise KRİTİK RİSK', () {
      store.bolum20 = Bolum20Model(
        binaIciYanginMerdiveniSayisi: 2,
        toplamDisariAcilanMerdivenSayisi: 1, // 1/2 >= 1/2 -> Oran OK
        lobiTahliyeMesafeDurumu: Bolum36Content.madde41MesafeUstunde, // Above limit
      );
    ...
    test('Bölüm 20 (Madde 41): Sprinkler varsa ve tahliye mesafesi <= 15m ise OLUMLU', () {
      store.bolum20 = Bolum20Model(
        binaIciYanginMerdiveniSayisi: 2,
        toplamDisariAcilanMerdivenSayisi: 1, // 1/2 >= 1/2 -> Oran OK
        lobiTahliyeMesafeDurumu: Bolum36Content.madde41MesafeAltinda, // Below limit
      );
      store.bolum9 = Bolum9Model(secim: Bolum9Content.tamKapsam); // Sprinkler var
      store.bolum36 = Bolum36Model(
        merdivenDegerlendirme: "",
      );
      ...
      final Color color = ReportEngine.getStatusColor(
        store.bolum36?.cikisKati, // Just a placeholder to trigger getStatusColor for sectionId 36
        sectionId: 36,
        store: store,
      );
    ...
    test('Bölüm 20 (Madde 41): 3 Merdiven varsa (ceil(3/2)=2) dışarı açılmalı', () {
      store.bolum20 = Bolum20Model(
    ...
    test('Bölüm 20 (Madde 41): 5 Merdiven varsa (ceil(5/2)=3) dışarı açılmalı', () {
      store.bolum20 = Bolum20Model(
    ...
    test('Bölüm 20 (Madde 41): Bağımsız Bodrum Merdivenleri Kontrolü (2 merdiven, 0 direkt)', () { 
      store.bolum20 = Bolum20Model(
    ...
    test('Bölüm 20 (Madde 41): Bağımsız Bodrum Merdivenleri Mesafe Kontrolü', () {
      store.bolum20 = Bolum20Model(
        isBodrumIndependent: true,
        bodrumNormalMerdivenSayisi: 2,
        bodrumToplamDisariAcilanMerdivenSayisi: 1, // 1 >= 1 -> Ratio OK
        bodrumLobiTahliyeMesafeDurumu: Bolum36Content.madde41MesafeUstunde, // Above limit
      );
