...
class _Bolum19ScreenState extends State<Bolum19Screen> {
  ...
  void _handleEngelSelection(ChoiceResult choice) {
    setState(() {
      ...
      if (choice.label == Bolum19Content.engelOptionA.label) current = [choice];
      else {
        current.removeWhere((e) => e.label == Bolum19Content.engelOptionA.label);
        if (current.any((e) => e.label == choice.label)) current.removeWhere((e) => e.label == choice.label);
  ...
  void _handleSelection(String type, ChoiceResult choice) {
    setState(() {
      ...
      if (type == 'yaniltici') {
        ...
        if (choice.label == Bolum19Content.yanilticiOptionB.label) _scrollToKey(_etiketKey);
        else _model = _model.copyWith(yanilticiEtiket: null);
      }
  ...
  bool _isReady() {
    if (_model.engeller.isEmpty || _model.levha == null || _model.yanilticiKapi == null) return false;
    if (_model.yanilticiKapi?.label == Bolum19Content.yanilticiOptionB.label && _model.yanilticiEtiket == null) return false;
    return true;
  }
        ...
        children: [
          QuestionCard(
            child: Column(
              ...
              children: [
                const Text("Kaçış yollarında aşağıdakilerden hangisi var?", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                const SizedBox(height: 12),
                ...[Bolum19Content.engelOptionA, Bolum19Content.engelOptionB, Bolum19Content.engelOptionC, Bolum19Content.engelOptionD].map((opt) => SelectableCard(choice: opt, isSelected: _model.engeller.any((e) => e.label == opt.label), onTap: () => _handleEngelSelection(opt))),
              ],
            ),
          ),
          ...
          if (_model.yanilticiKapi?.label == Bolum19Content.yanilticiOptionB.label) ...[
            ...
            _buildSoru("Bu kapıların üzerinde yazı var mı?", 'etiket', [Bolum19Content.etiketOptionA, Bolum19Content.etiketOptionB, Bolum19Content.etiketOptionC], _model.yanilticiEtiket),
          ],
        ],
      ),
    );
  ...
  Widget _buildSoru(String title, String key, List<ChoiceResult> options, ChoiceResult? selected) {
    return QuestionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 12), ...options.map((opt) => SelectableCard(choice: opt, isSelected: selected?.label == opt.label, onTap: () => _handleSelection(key, opt)))]));
  }
