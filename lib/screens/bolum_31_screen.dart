  ...
  void _checkAndRedirect() {
    ...
    if (b7?.hasTrafo == true) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const Bolum32Screen()));
      });
    }
  }
...
void _onNextPressed() {
    if (_hasTrafo) {
      ...
      if (_model.tip?.label == Bolum31Content.tipOptionB.label) {
        ...
        if (_model.sondurme == null) return _showError("Lütfen otomatik sistemleri seçiniz.");
      }
  ...
  Widget build(BuildContext context) {
    if (!_hasTrafo) return const Scaffold(body: Center(child: CircularProgressIndicator()));
      ...
      body: Column(
        children: [
          ModernHeader(
            ...
            subtitle: "Trafo odası güvenlik ve sızıntı analizi",
              ...
              child: Column(
                children: [
                  _buildSoru("Trafo odasının duvarları ve kapısı yangına dayanıklı mı?", 'yapi', 
                    [Bolum31Content.yapiOptionA, Bolum31Content.yapiOptionB, Bolum31Content.yapiOptionC, Bolum31Content.yapiOptionD], _model.yapi),

                  _buildSoru("Binanızdaki trafo 'Yağlı Tip' mi yoksa 'Kuru Tip' mi?", 'tip', 
                    [Bolum31Content.tipOptionA, Bolum31Content.tipOptionB, Bolum31Content.tipOptionC], _model.tip),

                  if (_model.tip?.label == Bolum31Content.tipOptionB.label) ...[
                    _buildInfoNote("Yağlı tip trafolar için sızıntı ve söndürme detayları doldurulmalıdır."),
                    _buildSoru("Trafonun altında yağ toplama çukuru ve ızgara var mı?", 'cukur', 
                      [Bolum31Content.cukurOptionA, Bolum31Content.cukurOptionB, Bolum31Content.cukurOptionC], _model.cukur),
                    _buildSoru("Trafo odasında otomatik yangın algılama veya söndürme sistemi var mı?", 'sondurme', 
                      [Bolum31Content.sondurmeOptionA, Bolum31Content.sondurmeOptionB, Bolum31Content.sondurmeOptionC], _model.sondurme),
                  ],

                  _buildSoru("Trafo odasının içerisinden su borusu geçiyor mu veya üst katında ıslak zemin var mı?", 'cevre', 
                    [Bolum31Content.cevreOptionA, Bolum31Content.cevreOptionB, Bolum31Content.cevreOptionC, Bolum31Content.cevreOptionD], _model.cevre),
                ],
              ),
            ),
          ),
