...
class _Bolum21ScreenState extends State<Bolum21Screen> {
  ...
  void _checkMandatory() {
    ...
    bool basementRisk = BinaStore.instance.bolum10?.bodrumlar.any((e) => 
      e != null && (e.label.contains("10-B") || e.label.contains("10-C") || e.label.contains("10-D") || e.label.contains("10-E"))
    ) ?? false;

    if (heightRisk) {
      setState(() {
        _isMandatory = true;
        _mandatoryReason = "Yapı yüksekliği 30.50m üzerinde olduğu için YGH zorunludur.";
      });
    } else if (basementRisk) {
      setState(() {
        _isMandatory = true;
        _mandatoryReason = "Bodrum katlardaki kullanım amacı (Ticari/Teknik) nedeniyle YGH zorunludur.";
      });
    } else {
      setState(() {
        _isMandatory = false;
      ...
      child: Column(
        children: [
          ...
          if (_model.varlik?.label == Bolum21Content.varlikOptionA.label) ...[
            _buildSoru("Hol içindeki kaplama malzemeleri yanmaz özellikte mi?", 'malzeme', [Bolum21Content.malzemeOptionA, Bolum21Content.malzemeOptionB, Bolum21Content.malzemeOptionC], _model.malzeme),
            _buildSoru("Hol kapıları duman sızdırmaz ve yangına dayanıklı mı?", 'kapi', [Bolum21Content.kapiOptionA, Bolum21Content.kapiOptionB, Bolum21Content.kapiOptionC], _model.kapi),
            _buildSoru("Hol içinde eşya (bisiklet, dolap vb.) var mı?", 'esya', [Bolum21Content.esyaOptionA, Bolum21Content.esyaOptionB, Bolum21Content.esyaOptionC], _model.esya),
          ],
        ],
      ),
    );
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(fontWeight: FontWeight.bold, color: _isMandatory ? Colors.orange.shade900 : Colors.blue.shade900),
                ),
                const SizedBox(height: 4),
                Text(_mandatoryReason, style: const TextStyle(fontSize: 12, color: Colors.black87)),
              ],
            ),
          ),
        ],
